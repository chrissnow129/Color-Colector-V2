...
export default function Nav(props) {
  ...
  return (
    <div key={randKeyNav}>
      ...
      >
        <span style={{ marginTop: "5rem" }} className={props.dark ? "bg-black" : "bg-white"} id="navigation__icon"></span>
      </label>
    ...
    <nav id='vertical' className={open ? "bg-white overflow-hidden bg-opacity-30 backdrop-filter backdrop-blur-sm backdrop-saturate-50 absolute rounded-tr-3xl rounded-br-3xl shadow-sm py-32 -top-0 h-screen w-72 transition-transform duration-700 ease-in-out" : 'h-screen overflow-hidden absolute w-72 py-32 -top-0 transition-transform duration-700 ease-in-out transform -translate-x-72'}>
    {routes
